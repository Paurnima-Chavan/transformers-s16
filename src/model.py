...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def attention(query, key, value, mask, dropout: nn.Dropout):
        ...
        if mask is not None:
            attention_scores.masked_fill_(mask == 0, torch.tensor(_MASKING_VALUE, dtype=torch.float32))
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, encoder_output, src_mask, tgt_mask):
        ...
        x = self.residual_connections[1](x, lambda x: self.self_attention_block(x, encoder_output, encoder_output,
                                                                                src_mask))
...
def build_transformer(src_vocab_size, tgt_vocab_size, src_seq_len: int, tgt_seq_len: int, d_model: int = 256,
                      N: int = 6, h: int = 8, dropout: float = 0.1, d_ff: int = 256) -> Transformer:   # d_model=512 , d_ff=2048
