...
def get_ds(config):
    ...
    # Sort the train_ds by the length of the sentence in it
    sorted_train_ds = sorted(train_ds_raw, key=lambda x: len(x['translation'][config['lang_src']]))
    filtered_sorted_train_ds = [k for k in sorted_train_ds if len(k['translation'][config['lang_src']]) < 120]
    filtered_sorted_train_ds = [k for k in filtered_sorted_train_ds if len(k['translation'][config['lang_tgt']]) < 120]
    filtered_sorted_train_ds = [k for k in filtered_sorted_train_ds if len(k['translation'][config['lang_src']])
                                + 10 > len(k['translation'][config['lang_tgt']])]
...
def collate_fn(batch):
    ...
    return {
        "encoder_inputs": torch.vstack(encoder_inputs),
...
def train_model(config):
    ...
    epochs = 90 # config['num_epochs'] 18
    scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=max_lr, steps_per_epoch=steps_per_epoch,
                                                    ...
                                                    div_factor=10, three_phase=True, final_div_factor=10,
    ...
    for epoch in range(initial_epoch, epochs):
        ...
        for batch in batch_iterator:
            ...
            decoder_input = batch['decoder_inputs'].to(device)
            encoder_mask = batch['encoder_mask'].to(device)
        ...
        # run the scheduler at the end of every epoch
